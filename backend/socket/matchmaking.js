...
const tryStartMatch = async (io, mode, fee) => {
  try {
    ...
    let colors;
    if (config.players === 2) {
      // Human user gets yellow (bottom-left), opponent/bot gets blue (top-right)
      const humanEntry = freshUsers.find(f => !f.entry.isBot);
      const botEntry = freshUsers.find(f => f.entry.isBot);
      if (humanEntry && botEntry) {
        // Ensure human always gets yellow
        colors = [];
        freshUsers.forEach(f => {
          colors.push(f.entry.isBot ? 'blue' : 'yellow');
        });
      } else {
        colors = ['yellow', 'blue'];
      }
    ...
    console.log(`[DEBUG] Creating game for ${key} with ${playerObjects.length} players...`);
    ...
    for (const { entry, user } of freshUsers) {
      try {
        ...
        console.error(`[DEBUG] Error deducting entry fee:`, err.message);
    ...
    console.log(`[DEBUG] Registering active game and emitting game_found`);
    ...
    freshUsers.forEach(({ entry, user }, i) => {
      ...
      if (entry.socket && typeof entry.socket.emit === 'function') {
        console.log(`[DEBUG] Emitting game_found directly via entry.socket for user ${user._id}`);
        entry.socket.emit('game_found', gameData);
      } else {
        console.log(`[DEBUG] Emitting game_found via io.to(${entry.socketId}) for user ${user._id}`);
    ...
    console.error(`[DEBUG] CRITICAL ERROR IN tryStartMatch:`, error);
  }
};

const handleJoinPool = async (socket, io, { mode, fee }) => {
  console.log(`[DEBUG] handleJoinPool called for user ${socket.userId} with mode=${mode}, fee=${fee}`);
