...
const DiceComponent = ({
  ...
  useEffect(() => {
    ...
    if (rolling) {
      ...
      Animated.parallel([Animated.loop(Animated.sequence([Animated.timing(spinXAnim, {
        toValue: 1,
        duration: 250,
        useNativeDriver: true
      }), Animated.timing(spinXAnim, {
        toValue: 2,
        duration: 300,
        useNativeDriver: true
      }), Animated.timing(spinXAnim, {
        ...
        duration: 0,
        useNativeDriver: true
      })])), Animated.loop(Animated.sequence([Animated.timing(spinYAnim, {
        toValue: 1,
        duration: 350,
        useNativeDriver: true
      }), Animated.timing(spinYAnim, {
        toValue: 2,
        duration: 250,
        useNativeDriver: true
      }), Animated.timing(spinYAnim, {
        ...
        duration: 0,
        useNativeDriver: true
      })])), Animated.loop(Animated.sequence([Animated.timing(spinZAnim, {
        toValue: 1,
        duration: 400,
        useNativeDriver: true
      }), Animated.timing(spinZAnim, {
        toValue: 2,
        duration: 300,
        useNativeDriver: true
      }), Animated.timing(spinZAnim, {
        ...
        duration: 0,
        useNativeDriver: true
      })])), Animated.loop(Animated.sequence([Animated.timing(jumpAnim, {
        toValue: 1,
        duration: 150,
        useNativeDriver: true
        ...
        duration: 200,
        useNativeDriver: true
      })])), Animated.loop(Animated.sequence([Animated.timing(shakeXAnim, {
        toValue: 4,
        duration: 60,
        useNativeDriver: true
      }), Animated.timing(shakeXAnim, {
        toValue: -4,
        duration: 60,
        useNativeDriver: true
      }), Animated.timing(shakeXAnim, {
      ...
      Animated.parallel([Animated.spring(spinXAnim, {
        toValue: Math.round(spinXAnim._value) + 1,
        friction: 5,
        tension: 80,
        useNativeDriver: true
      }), Animated.spring(spinYAnim, {
        toValue: Math.round(spinYAnim._value) + 1,
        friction: 4,
        tension: 60,
        useNativeDriver: true
      }), Animated.spring(spinZAnim, {
        toValue: Math.round(spinZAnim._value) + 1,
        friction: 5,
        tension: 70,
        useNativeDriver: true
  ...
  return <Animated.View style={[styles.glowWrapper, {
    opacity: glowAnim,
    ...
    transform: [{
      ...
      <TouchableOpacity onPress={onPress} disabled={disabled || rolling} activeOpacity={0.85} style={styles.touchable}>
        ...
        <Animated.View style={[styles.diceShadow, {
        width: size * 0.85,
        height: size * 0.85,
        borderRadius: size * (12 / 62),
        opacity: rolling ? jumpAnim.interpolate({
          inputRange: [0, 1],
          outputRange: [0.35, 0.12]
        }) : 0.35,
        transform: [{
          scale: rolling ? jumpAnim.interpolate({
            inputRange: [0, 1],
            outputRange: [0.92, 1.15]
          }) : 1
        }, {
          translateY: rolling ? jumpAnim.interpolate({
            inputRange: [0, 1],
            outputRange: [size * 0.05, size * 0.18]
          }) : size * 0.05
        }]
      }]} />

        {/* Dice Body */}
        <Animated.View style={[styles.diceOuter, {
        width: size,
        height: size,
        borderRadius: size * (12 / 62),
        borderWidth,
        ...
        transform: [{
          ...
          <LinearGradient colors={disabled ? ['#555', '#333'] : ['#FFFFFF', '#F8F9FA', '#E9ECEF']} style={[styles.dice, {
          padding
        }]}>
            {face.map((row, ri) => <View key={ri} style={styles.diceRow}>
                {row.map((dot, ci) => <View key={ci} style={[styles.dotSlot, {
              width: dotSlotSize,
              height: dotSlotSize
            }]}>
                ...
                borderWidth: size * (0.8 / 62)
                        ...
                        {!disabled && <View style={[styles.dotInsetHighlight, {
                  width: dotSize * 0.35,
                  height: dotSize * 0.35,
            ...
            {!disabled && <LinearGradient colors={['rgba(255,255,255,0.45)', 'rgba(255,255,255,0.08)', 'transparent']} start={{
            x: 0,
            y: 0
          }} end={{
            x: 0.8,
