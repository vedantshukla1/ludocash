...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, Animated, StatusBar, Alert, ActivityIndicator } from 'react-native';
...
const SelectFeeScreen = ({
  ...
  useEffect(() => {
    ...
    const loops = dotAnims.map((anim, i) => Animated.loop(Animated.sequence([Animated.delay(i * 200), Animated.timing(anim, {
      toValue: 1,
      duration: 300,
  ...
  const handleFindOpponent = async () => {
    ...
    if (balance < selectedFee) {
      CustomAlert.alert('Insufficient Balance', `You need ₹${selectedFee} to join. Your balance: ₹${balance.toFixed(0)}`, [{
        text: 'Add Money',
  ...
  const prize = selectedOption ? Math.floor(selectedOption.fee * selectedOption.players * (1 - selectedOption.cut / 100)) : 0;
  return <LinearGradient colors={GRADIENTS.background} style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.gridContainer}>
          ...
          return <TouchableOpacity key={opt.fee} style={[styles.feeCardGrid, isSelected && styles.feeCardSelectedGrid, !canAfford && styles.feeCardDisabledGrid]} onPress={() => {
            playSound("button_click");
            return !searching && canAfford && setSelectedFee(opt.fee);
          }} activeOpacity={0.85}>
                ...
                {isSelected && <View style={styles.feeCardCheckGrid}>
                    <Text style={styles.checkTextGrid}>✓</Text>
                  </View>}
                {!canAfford && <View style={styles.insufficientBadgeGrid}>
        ...
        {/* Prize summary */}
        {selectedOption && <View style={styles.summary}>
        ...
        {/* Searching state */}
        {searching && <View style={styles.searchingContainer}>
            <View style={styles.searchingDots}>
              {dotAnims.map((anim, i) => <Animated.View key={i} style={[styles.dot, {
            transform: [{
        ...
        {/* Action buttons */}
        {!searching ? <TouchableOpacity style={styles.findBtn} onPress={(...args) => {
        playSound("button_click");
        return handleFindOpponent(...args);
      }}>
            <LinearGradient colors={GRADIENTS.gold} style={styles.findBtnGrad}>
              <Text style={styles.findBtnText}>⚔️ Find Opponent</Text>
            </LinearGradient>
          </TouchableOpacity> : <TouchableOpacity style={styles.cancelBtn} onPress={(...args) => {
        playSound("button_click");
